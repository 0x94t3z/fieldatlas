# Field Atlas 1.1.7

This reviewed build bundles the Reference-aligned `questions-v2.json` device
benchmark and keeps the Field Atlas Reference knowledge pack. It was built
from the public repository and installed on the physical Infinix SMART 20 /
X6840 running Android 16 (API 36).

| Artifact | Value |
| --- | --- |
| Package | `xyz.fieldatlas` |
| Version | `1.1.7` (`versionCode` 10) |
| Signed APK | `app-release.apk` |
| APK SHA-256 | `f0827b035e87775febabfebab0b41fea96f251c648b975a90ca7df19cfba0c06` |
| Benchmark | `benchmarks/questions-v2.json` |
| Knowledge | `fieldatlas-reference` 1.0.0 |

The APK is signed with the project release key and was installed with ADB on
the device. The v2 benchmark has not yet been exported from this 1.1.7 device
installation; the prior 1.1.6 export remains explicitly a v1 run.
