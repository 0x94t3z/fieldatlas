...
Report every response, category mean, overall mean, and the offline-to-baseline ratio. A zero baseline denominator produces `null`, never infinity. The scorer always emits `not_an_issuer_determination: true`; only the bounty issuer decides whether the greater-than-50-percent bar is met.

## Capture a local-device run

1. Install the verified model and knowledge packs, load the model from Research, then open **Proof → Open device benchmark**.
...
The app never runs ordinary research and a benchmark row concurrently. Exports are evidence, not a quality score or a claim that the bounty threshold was met.
